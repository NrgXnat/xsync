...
//		Configuration config = _configService.getConfig("xsync", project.getId());
	...
	@RequestMapping(value = "/projects/{projectId}/presyncanonymization", method = RequestMethod.PUT)
	public ResponseEntity<String> addDICOMAnonymization(@PathVariable("projectId") final String projectId,@RequestBody String anonymizationScript) {
		UserI user = getSessionUser();
		try {
	        XnatProjectdata project = XnatProjectdata.getProjectByIDorAlias(projectId, user, false);
            if (project == null) {
	...
	@RequestMapping(value = "/projects/{projectId}/presyncanonymization", method = RequestMethod.GET)
