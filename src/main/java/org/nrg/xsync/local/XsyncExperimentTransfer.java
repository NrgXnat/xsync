...
public class XsyncExperimentTransfer {
            ...
            if (stored) {
                ...
                for (XnatAbstractresourceI resource : orig.getResources_resource()) {
                    if (projectSyncConfiguration.isSubjectAssessorResourceToBeSynced(assessor.getXSIType(), resource.getLabel())) {
                        ...
                        resourceSyncItem.setFileSize(resource.getFileSize() != null ? resource.getFileSize() : 0L);
    ...
    private boolean storeXar(XnatImagesessiondata orig, String targetproject, XnatSubjectdata targetsubject, XnatImagesessiondata target, boolean updateSyncAssessor, SessionToSync sessionToSync) throws XsyncRemoteConnectionException {
        ...
        try {
            ...
            log.debug("BUILDING XAR FILE: targetProject={}, targetSubject={}, target={}, file={}",
                    targetproject, targetsubject.getLabel(), target.getLabel(), xar.getName());
            ...
            if (stored) {
                ...
                if (remote_id != null && remote_id.matches("^.*_S[0-9]*$")) {
                    log.error("ERROR: Experiment appears to have been assigned a subject accession number!!! {}", remote_id);
                }
                ...
                if (remote_id == null) {
                    throw new XsyncStoreException("Could not locate Accession Id for " + target.getLabel() +
                ...
                for (XnatAbstractresourceI xnatAbstractresourceI : resources) {
                    final XnatAbstractresource resource = (XnatAbstractresource) xnatAbstractresourceI;
                    ...
                    if (resource.getFileCount() != null && resource.getFileSize() != null) {
                        ...
                        if (hasBeenSkipped) {
                            ...
                            pushImagingSessionResource(targetproject, targetsubject, orig, targetWithRemoteId, expSyncItem, resource);
                        }
                    }
                }
                ...
                for (XnatImagescandataI xnatImagescandataI : scans) {
                    final XnatImagescandata scan = (XnatImagescandata) xnatImagescandataI;
                ...
                for (XnatImageassessordataI assessor : assessors) {
                    final XnatImageassessordata origAss = (XnatImageassessordata) assessor;
                    ...
                    for (final XnatAbstractresourceI res : targetAss.getResources_resource()) {
                        experimentMapper.modifyExptResource(res, orig, true);
                    }
                ...
                // Verify
                verifySync(targetproject, expSyncItem, orig, targetsubject, updateSyncAssessor);
                _syncStatusService.registerCompletedExperiment(_localProject.getId(), orig.getLabel(), orig.getXSIType());

            } else if (connectionResponse.getResponse().getStatusCode().value() == HttpStatus.INTERNAL_SERVER_ERROR.value()) {
                ...
                if (!exists) {
                    ...
                    expSyncItem.setSyncStatus(XsyncUtils.SYNC_STATUS_FAILED);
                    expSyncItem.setMessage("Subject " + localSubject.getLabel() + " experiment " +
                            orig.getLabel() + " could not be synced. ");
                    stored = false;
                } else {
            ...
            expSyncItem.setSyncStatus(XsyncUtils.SYNC_STATUS_FAILED);
            expSyncItem.setMessage("Subject " + localSubject.getLabel() + " experiment " +
                    orig.getLabel() + " could not be synced. " + e.getMessage());
            stored = false;
        } finally {
            if (remote_id != null && !remote_id.isEmpty()) {
                saveSyncDetails(orig.getId(), remote_id, expSyncItem.getSyncStatus(), expSyncItem.getXsiType());
            }
        ...
        try {
            ...
            if (updateSyncAssessor) {
                try {
                    ...
                    log.error("Failed to update sync assessor", e);
                }
            }
    ...
    private boolean removeSingleAssessor(XnatImagesessiondata target) {
        if (!target.getAssessors_assessor().isEmpty()) {
    ...
    private boolean removeSingleScan(XnatImagesessiondata target) {
        if (!target.getScans_scan().isEmpty()) {
    ...
    private boolean removeSingleResource(XnatImagesessiondata target) {
        if (!target.getResources_resource().isEmpty()) {
    ...
    File buildxar(XnatImagesessiondata orig, String targetproject, XnatSubjectdata targetsubject, XnatImagesessiondata target) throws Exception {
        ...
        try {
            ...
            if (target.getScans_scan() != null && !target.getScans_scan().isEmpty()) {
    ...
    File buildImagingSessionXar(XnatImagesessiondata orig, String targetproject, XnatSubjectdata targetsubject, XnatImagesessiondata targetOrig) throws Exception {
        ...
        try {
            ...
            if (targetExperiment.getResources_resource() != null && !targetExperiment.getResources_resource().isEmpty()) {
                removeResources(targetExperiment);
            }

            if (targetExperiment.getScans_scan() != null && !targetExperiment.getScans_scan().isEmpty()) {
    ...
    File buildImagingScanXar(XnatImagesessiondata orig, String targetProject, XnatImagesessiondata target,
        ...
        try {
            ...
            throw new Exception("Unable to retrieve/save scan XML", e);
        }
        return xarFile;
    }
    ...
    private void pushImagingSessionResource(String remoteProjectId, XnatSubjectdata targetsubject, XnatImagesessiondata orig, XnatImagesessiondata target, ExperimentSyncItem expSyncItem, XnatAbstractresourceI resource) {
        ...
        if (resource.getFileSize() != null)
            resourceSyncItem.setFileSize((Long) resource.getFileSize() > 0 ? resource.getFileSize() : 0L);
        else
            resourceSyncItem.setFileSize(0L);
        try {
