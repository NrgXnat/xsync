...
public class QueryResultUtil {
	...
	public String getQueryForFetchingSubjectsModifiedSinceLastSync() {
		String query = "select s.id, s.label,s.project, sm.status, sm.last_modified,xsi.sync_start_time from xnat_subjectdata_meta_data sm ";
		...
		query += " left join xsync_xsyncprojectdata xp ON xp.project_id=p.id ";
	...
	public String getQueryForFetchingSubjectsWhoseExperimentsMarkedOKSinceLastSync(boolean skipSubjectIdCheck) {
		...
		query += " left join xsync_xsyncprojectdata xp ON xp.project_id=p.id ";
		query += " left join xsync_xsyncinfodata xsi ON xp.syncinfo_xsync_xsyncinfodata_id=xsi.xsync_xsyncinfodata_id ";
		query += " where project=:"+ PROJECT_QUERY_PARAMETER_NAME+" and  xok.sync_status is NULL and xok.oktosync=1 and xok.authorized_time > xsi.sync_start_time  ";
	...
	public String getQueryForFetchingProjectResourcesModifiedSinceLastSync() {
		String query = "select  a.label, p.id, am.status, am.last_modified,xsi.sync_start_time from xnat_abstractresource a ";
		...
		query += " left join xsync_xsyncprojectdata xp ON xp.project_id=p.id ";
		query += " left join xsync_xsyncinfodata xsi ON xp.syncinfo_xsync_xsyncinfodata_id=xsi.xsync_xsyncinfodata_id  ";
		query += " where p.id=:"+PROJECT_QUERY_PARAMETER_NAME+" and am.row_last_modified > xsi.sync_start_time  ";
		query += " UNION "; 
		query += " select ah.label, p.id, am.status, am.last_modified,xsi.sync_start_time from xnat_abstractresource_meta_data am  ";
		...
		query += " left join xsync_xsyncprojectdata xp ON xp.project_id=p.id  ";
		query += " left join xsync_xsyncinfodata xsi ON xp.syncinfo_xsync_xsyncinfodata_id=xsi.xsync_xsyncinfodata_id ";
		query += "  where am.status='"+ DELETE_STATUS + "' and prh.xnat_projectdata_id=:"+PROJECT_QUERY_PARAMETER_NAME+" and am.row_last_modified > xsi.sync_start_time "; 
		return query;
	}
	...
	public String getQueryForFetchingSubjectResourcesModifiedSinceLastSync() {
		String query = "select  a.xnat_abstractresource_id,a.label, p.id, am.status, am.last_modified,xsi.sync_start_time from xnat_abstractresource a ";
		query += " left join xnat_abstractresource_meta_data am ON a.abstractresource_info = am.meta_data_id ";
		...
		query += " left join xsync_xsyncprojectdata xp ON xp.project_id=p.id ";
		query += " left join xsync_xsyncinfodata xsi ON xp.syncinfo_xsync_xsyncinfodata_id=xsi.xsync_xsyncinfodata_id   ";
		query += " where s.id=:"+ this.SUBJECT_QUERY_PARAMETER_NAME + " and p.id=:"+PROJECT_QUERY_PARAMETER_NAME+" and am.row_last_modified > xsi.sync_start_time ";  
		return query ;
	}

	public String getQueryForFetchingSubjectResourcesDeletedSinceLastSync() {
		String query = " select ah.xnat_abstractresource_id,ah.label, p.id, am.status, am.last_modified,xsi.sync_start_time from xnat_abstractresource_meta_data am  "; 
		query += " left join xnat_abstractresource_history ah ON ah.abstractresource_info=am.meta_data_id   ";
		...
		query += " left join xsync_xsyncprojectdata xp ON xp.project_id=p.id ";  
		query += " left join xsync_xsyncinfodata xsi ON xp.syncinfo_xsync_xsyncinfodata_id=xsi.xsync_xsyncinfodata_id "; 
		query += "  where s.id=:" + this.SUBJECT_QUERY_PARAMETER_NAME + " and am.status='"+ DELETE_STATUS + "' and p.id=:"+PROJECT_QUERY_PARAMETER_NAME+" and am.row_last_modified > xsi.sync_start_time ";
		return query;
	}	
	...
	public String getQueryForFetchingSubjectExperimentsModifiedSinceLastSync() {
		String query = "select e.id,e.label,xdme.element_name,e.project,em.status,em.last_modified, xsi.sync_start_time from xnat_experimentdata e ";
		...
		query += " left join xsync_xsyncprojectdata xp ON xp.project_id=p.id ";
		query += " left join xsync_xsyncinfodata xsi ON xp.syncinfo_xsync_xsyncinfodata_id=xsi.xsync_xsyncinfodata_id ";  
		query += " where sa.subject_id=:" +  this.SUBJECT_QUERY_PARAMETER_NAME + " and  p.id=:"+ PROJECT_QUERY_PARAMETER_NAME +  " and em.row_last_modified > xsi.sync_start_time and e.id in (:"+EXPERIMENT_IDS+") ";
		return query;
	}
	...
	public String getQueryForFetchingSubjectExperimentsDeletedSinceLastSync() {
		...
		String query = "select eh.id,eh.label,xdme.element_name,eh.project,em.status,em.last_modified, xsi.sync_start_time from xnat_experimentdata_meta_data em ";
		...
		query += " left join xsync_xsyncprojectdata xp ON xp.project_id=p.id ";
		query += " left join xsync_xsyncinfodata xsi ON xp.syncinfo_xsync_xsyncinfodata_id=xsi.xsync_xsyncinfodata_id ";  
		query += " where sa.subject_id=:" + this.SUBJECT_QUERY_PARAMETER_NAME +  " and p.id=:"+ PROJECT_QUERY_PARAMETER_NAME +  " and  em.status='"+ DELETE_STATUS + "' and em.row_last_modified > xsi.sync_start_time  ";
		return query;
	}
